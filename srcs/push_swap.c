...
static void set_stack_indexes(t_stack *stack, int *sorted_numbers, int size)
{
    t_node *tmp;
    ...
    while (tmp)
    {
        tmp->s_index = index_of(tmp->data, sorted_numbers, size);
        tmp = tmp->next;
    }
}
...
void init_stack(t_stack *stack_a, t_stack *stack_b, int *nb, int c)
{
    ...
    insertion_sort(nb, c);
    set_stack_indexes(stack_a, nb, c);
}
